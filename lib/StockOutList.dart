...
import 'package:standoutinventoryapplication/TaskProgressPage.dart';
import 'package:standoutinventoryapplication/UserProfile.dart';

class StockOutList extends StatefulWidget {
  const StockOutList({Key? key}) : super(key: key);

  @override
  _StockOutListState createState() => _StockOutListState();
}

class _StockOutListState extends State<StockOutList> {
  String _selectedOption = 'New Task';
  late CollectionReference<Map<String, dynamic>> _stockOutTasksCollection;
  ...
  void initState() {
    super.initState();
    _getCurrentUserId();
    _stockOutTasksCollection = FirebaseFirestore.instance.collection('StockOutTasks');
  }
  ...
  Widget _buildNewTaskList() {
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      stream: _stockOutTasksCollection
  ...
  Widget _buildProgressionList() {
    return StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
      stream: _stockOutTasksCollection
          .where('assignedStorekeeperId', isEqualTo: _currentUserId)
          .where('status', whereIn: ['Packing', 'Delivery'])
          .snapshots(),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return Center(child: CircularProgressIndicator());
