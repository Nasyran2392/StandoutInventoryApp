...
class StoreInItemsPage extends StatefulWidget {
  final String stockInTaskId;

  StoreInItemsPage({required this.stockInTaskId});

  @override
  _StoreInItemsPageState createState() => _StoreInItemsPageState();
}

class _StoreInItemsPageState extends State<StoreInItemsPage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Store-In Items'),
      ),
      body: StreamBuilder<DocumentSnapshot<Map<String, dynamic>>>(
        stream: FirebaseFirestore.instance.collection('StockInTasks').doc(widget.stockInTaskId).snapshots(),
                        ...
                        child: ListTile(
                          ...
                          subtitle: Column(
                            ...
                            children: [
                              ...
                              Text(
                                'Store-In Quantity: $quantityInventory',
                                style: TextStyle(fontSize: 14),
                              ),
                            ],
                          ),
                                      ...
                                      return AlertDialog(
                                        ...
                                        content: Column(
                                          ...
                                          children: [
                                            ...
                                            Text('Store-In Quantity: $quantityInventory'),
                                          ],
                                        ),
                                        actions: <Widget>[
                                          TextButton(
                                  ...
                                  showDialog(
                                    ...
                                    builder: (BuildContext context) {
                                      return AlertDialog(
                                        ...
                                        content: Text('Do you place items with enough quantity and right placement?'),
                                        actions: <Widget>[
                                          TextButton(
                                            onPressed: () {
                                              setState(() {
      ...
      floatingActionButton: isAllConfirmed ? FloatingActionButton.extended(
        onPressed: () {
          ...
          _changeStatusToWaiting(context);
        },
        label: Text('Proceed'),
  ...
  void _changeStatusToWaiting(BuildContext context) async {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          title: Text('Confirm?'),
          content: Text('Are you sure you want to proceed this task?'),
          actions: <Widget>[
            TextButton(
              onPressed: () async {
                try {
                  // Update the status attribute to "Delivery" in Firestore
                  await FirebaseFirestore.instance.collection('StockInTasks').doc(widget.stockInTaskId).update({
                    'status': 'Waiting',
                  });
                  // Send email to admin
                  _sendEmail();
