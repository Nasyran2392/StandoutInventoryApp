...
import 'package:standoutinventoryapplication/PickUpItemsPage.dart';
import 'package:standoutinventoryapplication/StoreInItemsPage.dart';
import 'package:http/http.dart' as http;
import 'dart:convert';
import 'package:logger/logger.dart';

class StockInProgressPage extends StatefulWidget {
  final String stockInTaskId;

  StockInProgressPage({required this.stockInTaskId});

  @override
  _StockInProgressPageState createState() => _StockInProgressPageState();
}
...
class _StockInProgressPageState extends State<StockInProgressPage> {
              ...
              Expanded(
                ...
                child: Stepper(
                  ...
                  steps: [
                    ...
                    _buildStep('Pick-Up', status),
                    _buildStep('Store-In', status),
                    _buildStep('Waiting', status),
                    _buildStep('Approved', status),
                  ],
                ),
              ),
              if (_currentStatus == 'Pick-Up' || _currentStatus == 'Store-In')
                ElevatedButton(
                  onPressed: () {
                    _rejectTask();
                  },
  ...
  void _handleStepContinue() {
    ...
    } else if (_currentStatus == 'Pick-Up') {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => PickUpItemsPage(stockInTaskId: widget.stockInTaskId),
        ),
      );
    } else if (_currentStatus == 'Store-In') {
      Navigator.push(
        context,
        MaterialPageRoute(
          builder: (context) => StoreInItemsPage(stockInTaskId: widget.stockInTaskId),
        ),
      );
    } else {
      _proceedWithTask();
  ...
  void _proceedWithTask() async {
    setState(() {
      switch (_currentStatus) {
        case 'Pending':
          _currentStatus = 'Pick-Up';
          break;
        case 'Pick-Up':
          _currentStatus = 'Store-In';
          break;
        case 'Store-In':
    ...
    if (_currentStatus == 'Pick-Up') {
      // Update status before sending email
      _updateStatus(_currentStatus);

      // Send email after updating status
      await _sendEmail();
    } else {
      // For other statuses, update status directly
      _updateStatus(_currentStatus);
    }
  }
  ...
  Future _sendEmail() async {
    ...
    // Fetch user details using assignedStorekeeperId
  ...
  void _updateStatus(String status, {String? rejectedDetails}) {
    ...
    FirebaseFirestore.instance.collection('StockInTasks').doc(widget.stockInTaskId).update(updateData);
  }
  ...
  //       await FirebaseFirestore.instance.collection('StockInTasks').doc(widget.stockInTaskId).get();
